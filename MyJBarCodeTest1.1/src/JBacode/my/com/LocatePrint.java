...
//        printX = 595 / 2; // 给定字符点阵，X页面正中 
//        printY = 842 / 2; // 给定字符点阵，Y页面正中
//        float printMX = printFontCount * printFontSize / 2;// 打印到正中间 
//        float printMY = printFontSize / 2;// 打印到正中间
//        ImageIcon[] icon = new ImageIcon[sum];
//        Image[] image = new Image[sum];
        if(sum ==1){
            ImageIcon icon=new ImageIcon("E://360data//重要数据//桌面//1/"+str[0]+".jpeg");
        ...
        for(int i = 0;i<sum;i++){
        jishu = i+1;
        ImageIcon icon=new ImageIcon("E://360data//重要数据//桌面//1/"+str[i]+".jpeg");
        Image image=icon.getImage();
